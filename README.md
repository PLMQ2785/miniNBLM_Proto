...
`인덱싱 완료`가 되면 왼쪽 목록에서 질문할 PDF를 선택한 후 질의합니다. 검색과
후속 질문은 선택한 문서 안으로 제한됩니다. 문서와 대화는 로그인한 사용자별로
분리되며, 답변 아래의 페이지 버튼을 누르면 원본 PDF의 해당 페이지가 열립니다.
대화는 선택 문서에 귀속되어 저장되고, 로그인하거나 새로고침하면 가장 최근
대화와 질의 문서가 함께 복원됩니다. 작업공간 상단에서 새 대화를 시작하거나
...
- 로그인 사용자가 선택한 indexed 문서 하나로 제한하는 문서별 RAG 검색
...
- 문서별 대화 세션 저장, 최근 대화·질의 문서 자동 복원과 직전 대화 기반 후속 검색 질의 재작성
