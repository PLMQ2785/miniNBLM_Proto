...
load_config() {
  ...
  LLM_SECRETS_DIR_VALUE="$(absolute_path "${LLM_SECRETS_DIR:-$(env_value LLM_SECRETS_DIR data/secrets/llm)}")"
  GEMMA4_API_KEY_VALUE="${GEMMA4_API_KEY:-$(env_value GEMMA4_API_KEY EMPTY)}"
  ...
  mkdir -p "$RUNTIME_DIR" "$LOG_DIR" "$HF_HOME_DIR" "$UPLOAD_DIR_VALUE" "$LLM_SECRETS_DIR_VALUE"
}
...
validate_api_config() {
  ...
  if ! env LLM_ENDPOINTS_FILE="$LLM_ENDPOINTS_FILE_VALUE" \
      LLM_SECRETS_DIR="$LLM_SECRETS_DIR_VALUE" GEMMA4_API_KEY="$GEMMA4_API_KEY_VALUE" \
      "$NATIVE_PYTHON" -c \
